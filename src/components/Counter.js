...
export default function Counter(props) {
    ...
    return (
        <div className="alert alert-info mt-3  d-flex flex-column align-items-center w-50 mx-auto p-4 border border-info border-4 rounded">
            ...
            <div className="d-flex flex-wrap justify-content-center">
                <button className="btn btn-outline-danger rounded-pill m-2 px-3" onClick={props.onReset}>Reset Count</button>
                <button className="btn btn-outline-danger rounded-pill m-2 px-4" onClick={props.onResetAll}>Reset All</button>
            </div>
        </div>
    )
}
